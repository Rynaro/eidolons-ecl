...
```json
{
  ...
  "parent_id":  null,
  "from": { "eidolon": "atlas",   "version": "1.5.1"  },
  "to":   { "eidolon": "spectra", "version": "4.3.1" },
  ...
  "artifact": {
    "kind": "scout-report",
    "schema_version": "2.0",
    "path": ".eidolons/atlas/output/scout-report-2026-05-13.md",
  ...
  "context_delta": {
    "token_budget": 4000,
    "tokens_used": 3420,
    "input_handles": [".spectra/v2.0-phase2c.md"],
  ...
  "ise": {
    ...
    "provenance": {
      "methodology_version": "atlas-1.5.1",
      "tool_surface": ["read", "grep", "bash"],
      "lateral_consults": []
    },
...
ECL defines exactly **ten** performatives. The set is **closed**;
introducing a new performative requires a SemVer bump.
...
### §6.5 — ISE trust hierarchy (v2.0)

The **ISE** (Intent, Source, Entitlement) block is a new OPTIONAL top-level
field introduced at v2.0. It composes with — and does NOT replace —
`constraints.trust_level` (§6.3). `trust_level` answers "how cautious should
the receiver be?"; `ise.*` answers "what specific authorities does the emitter
claim, and what is the receiver permitted to do?". Neither field deprecates
the other. The §6.3.2 MUST-NOT-bypass clause (trust_level=high SHALL NOT
bypass other normative constraints) applies with equal force to `ise.*`.

#### §6.5.1 — Emitter rules

§6.5.1 — **MAY**: `ise` is OPTIONAL on emit at v2.0. Envelopes without it
remain conformant (gate S-1 is not-applicable when the block is absent).

§6.5.2 — **MUST**: when `ise` is present, `ise.assertion_grade` is REQUIRED
inside the block. The four valid values are:

| Value | Meaning |
|---|---|
| `unverified` | Emitted without internal checks. |
| `self-attested` | Emitter ran its own validation. |
| `validated` | Emitter ran spec-mandated gates. |
...
|---|---|---|
| `auto_route` | `true` | Receiver MAY hand the envelope to the next contracted edge without operator confirm. |
| `auto_merge` | `false` | Receiver MAY merge the artefact into a mainline branch without operator confirm. |
| `auto_deploy` | `false` | Receiver MAY deploy/publish without operator confirm. |

#### §6.5.4 — Receiver rules

§6.5.4 — **MUST**: receivers SHALL accept envelopes without `ise`
...
minor release. v1.1 SHALL accept v1.0 envelopes unchanged.

**v2.0 receivers SHALL accept v1.x envelopes** (any `envelope_version` matching
`^1\.[012](\.\d+)?$`) through **2027-05-13** (12 months from v2.0.0 publication).
...
`^1\.[012](\.\d+)?$` does not match `envelope_version: "2.0"`). This is expected
behaviour, not a v2.0 defect. See `docs/migration-v1-to-v2.md` for the full
migration guide.
...
open drifts and candidates. None are formally open at v2.0.0; see the
