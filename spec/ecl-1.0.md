...
`^1\.0(\.\d+)?$` for ECL v1.0 conformance.
...
|---|---|---|
| `sha256` | Default for `trust_level ∈ {low, standard}`. | Lowercase hex digest of the SHA-256 hash of the payload file's bytes at emit time. |
| `hmac-sha256` | OPTIONAL; RECOMMENDED for `trust_level = high`. | Lowercase hex digest of HMAC-SHA-256 of the payload bytes, keyed by a per-thread shared secret supplied via the `ECL_HMAC_KEY` environment variable. |
...
warn-only window per drift. Drifts are designated `D-1`, `D-2`, …. None
exist at v1.0.0; entries will be added in CHANGELOG.md as discovered.
